...
public class CreatureHandler
{
	...
	*/
	public void updateCreatures() {
		...
		List<Creature> _newAnimals = new ArrayList<>();
		for(Creature c : _creatures) {
			...
			if(c instanceof Spawner) {
				Creature ch = ((Spawner) c).spawnNewCreature();
				if(ch != null){
					_newAnimals.add(ch);
				}
			}

			if(c instanceof Growth) {
				Creature ca = ((Growth) c).grow();
				if(ca != null){
					_newAnimals.add(ca);
				}
			}
